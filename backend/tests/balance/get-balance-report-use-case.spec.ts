import { mockBalanceRepository } from '@/mocks/balance-repository.mock';
...
describe('GetBalanceReportUseCase', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        useCase = new GetBalanceReportUseCase(mockBalanceRepository);
    });

    test('should return balance report with amounts converted from cents to reais and computed balance', async () => {
        mockBalanceRepository.getAggregatesByCategory.mockResolvedValueOnce([
            { categoryId: 'c1', count: 2, amount: 12345 }, // R$123.45
            { categoryId: 'c2', count: 1, amount: 500 }, // R$5.00
        ] as any);
        mockBalanceRepository.getAggregatesByType.mockResolvedValueOnce([
    ...
    test('should call repository with parsed dates and provided userId', async () => {
        ...
        expect(
            mockBalanceRepository.getAggregatesByCategory,
        ).toHaveBeenCalledTimes(1);
        expect(mockBalanceRepository.getAggregatesByType).toHaveBeenCalledTimes(
            1,
        );

        const [userId1, start1, end1] =
            mockBalanceRepository.getAggregatesByCategory.mock.calls[0];
        const [userId2, start2, end2] =
            mockBalanceRepository.getAggregatesByType.mock.calls[0];
